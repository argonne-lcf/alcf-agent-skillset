...
# ///
"""
30-task hostname+sleep stress on the Polaris facility multiuser Globus
Compute endpoint, using MpiExecLauncher across 2 physical nodes. Mirrors
the working Crux config: place=scatter via scheduler_options to force one
chunk per physical node.
"""
...
print("submitting 30 tasks to Polaris (MpiExecLauncher, 2 nodes x 4 workers) ...", flush=True)
with Executor(
    endpoint_id=POLARIS,
...
import json
with open("/tmp/polaris_throughput_mpi.json", "w") as fh:
...
ax1.set_title("Polaris throughput (MpiExecLauncher, place=scatter): 30 tasks, 4 workers x 2 nodes")
...
fig.tight_layout()
out = "/tmp/polaris_throughput_mpi.png"
